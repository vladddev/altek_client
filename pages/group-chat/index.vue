<template>
  <v-card width="100%" class="h-100">
    <div class="d-flex h-100">
      <v-col class="pd-0 h-100" cols="5">
        <div class="chat-panel">
          <v-list-item class="h-100">
            <v-list-item-avatar>
              <!--              <v-img src="https://flynow.ru/wp-content/themes/flynow-new/images/default-user.png"></v-img>-->
              <v-img v-if="$store.state.currentUser.avatar !== undefined"
                :src="$store.state.backendUrl + $store.state.currentUser.avatar"></v-img>
            </v-list-item-avatar>

            <v-list-item-content>
              <v-list-item-title>
                <v-row class="mr-0">
                  {{ $store.state.currentUser.first_name }} {{ $store.state.currentUser.last_name }}
                </v-row>
              </v-list-item-title>
            </v-list-item-content>
          </v-list-item>
        ...
        <v-list class="pd-0 scroll-view" style="overflow-y: overlay; height: calc(100% - 75px)">
          <template v-if="chats.length > 0" v-for="(chat, index) in chats">

            <div :key="chat.id">
              <v-list-item @click="openChat(chat)" two-line
                ...
                <v-list-item-content>
                  ...
                  <v-list-item-title>
                    ...
                    <v-row class="mr-0">
                      <template v-for="(user, user_index) in chat.users">
                        <span :key="user_index">
                          <template v-if="user_index==0">
                            {{ user.first_name }}
                          </template>
          ...
          <v-progress-circular v-else :width="3" color="red" indeterminate></v-progress-circular>
        </v-list>
      </v-col>
      <v-col class="pd-0 h-100" cols="7">
        <div class="chat-window h-100">
          <div v-if="mode == 'chat'" style="height: 100%">
                ...
                <v-list-item-content>
                  ...
                  <v-list-item-title>
                    ...
                    <v-row class="mr-0">
                      ...
                      <v-col cols="2">
                        <v-btn text color="blue" @click="mode = 'usersAdd'">Users</v-btn>
                      </v-col>
              ...
              <div style="display: flex; flex-direction: column; width: 100%">
                <div class="files-wrapper">
                  <div class="files-item" v-for="(file, index) in files" :key="index">
                    ...
                    <img v-if="file.type.type == 'docx'" src="https://altekloads.com/icons/docs.svg"
                      class="files-item__sheet" alt="">
                  </div>
                </div>
          ...
          <div v-else-if="mode == 'usersAdd'">
                ...
                <v-list-item-content>
                  ...
                  <v-list-item-title>
                    ...
                    <v-row class="mr-0">
                      ...
                      <v-col cols="2">
                        <v-btn text color="blue" @click="mode = 'chat'">Chat</v-btn>
                      </v-col>
            ...
            <v-container>
              <v-row>
                ...
                <v-col cols="12">
                  <v-spacer></v-spacer>
                  <v-btn @click="saveChatUses">Save</v-btn>
                </v-col>
              </v-row>
            </v-container>
          </div>
          <div v-else class="chat-empty-msg-block">
            <v-img max-height="121" max-width="121" contain src="/images/chat-empty.svg"></v-img>
  ...
  export default {
    ...
    methods: {
      ...
      sendMessage() {
        ...
        // axios({
        //   method: 'put',
        //   url: self.$store.state.backendUrl + '/backend/api/users/current/chats/41/',
        //   data: {
        //     users: [7],
        //     action: 'add'
        //   },
      ...
      saveChatUses() {
        ...
        axios({
          method: 'put',
          url: self.$store.state.backendUrl + '/backend/api/users/current/chats/' + self.current_chat_id + '/',
          data: {
            users: self.newMembers
          },
          headers: {
            'Authorization': 'JWT ' + accessToken
          },
        ...
        .then(function (response) {
          self.openChat(self.current_chat_id)
        })
        .catch(function (error) {
          console.log(error);
        });
      ...
      openChat(chat, chattingAddingMode = true) {
        ...
        axios({
            method: 'get',
            url: self.$store.state.backendUrl + '/backend/api/users/current/chats/' + chat.id + '/',
            params: {
      ...
      connect(chat_id) {
        ...
        this.chatSocket = new WebSocket(this.$store.state.backendSocketUrl + '/ws/chat/' + chat_id + '/?access_token=' +
          accessToken)
      ...
      search() {
        ...
        axios({
          method: 'get',
          url: self.$store.state.backendUrl + '/backend/api/users/current/chats/?search=' + self.searchField.trim(),
          headers: {
            'Authorization': 'JWT ' + accessToken
          },
        })
    ...
    created() {
      ...
      axios({
          method: 'get',
          url: self.$store.state.backendUrl + '/backend/api/users/my?all=true',
          headers: {
            'Authorization': 'JWT ' + accessToken
          },
        })
    ...
    mounted() {
      ...
      this.$store.commit('setMessages', 0)
